...
GUI.mailling.sendMail("youssef.achour1@esprit.tn");
    ...
    private void ViewProduit(ActionEvent event) throws SQLException {
      ...
      List<Produit> getProduitbyIdC(int idC) throws SQLException{
        List<Produit> arr = new ArrayList<>();
         try {
        PreparedStatement pre = cnx.prepareStatement("SELECT nomP , prixP,descP, dispoP, couleurP,quantiteP,tailleP from produit p , commande c where c.idP=p.idP and c.idC=?;"); //ORDER BY P asc
         pre.setInt(1, idC);
         ResultSet rs = pre.executeQuery();
             ...
             while(rs.next()){
                     ...
                     Produit m= new Produit(nomP,prixP,descP,dispoP,couleurP,quantiteP,tailleP);
                    arr.add(m);
            }
        } catch (Exception e) {
