...
public class ReservationCrudController implements Initializable {
    ...
    @FXML
    private TableColumn<Reservation, String> disponibleCOL;

    @FXML
    private TableColumn<Reservation, Time> heureCOL;

    @FXML
    private TableColumn<Reservation, Integer> idCOL;

    @FXML
    private TableColumn<Reservation, Double> prixCOL;
    ...
    @FXML
    private ChoiceBox<String> disponibleCB;
    ...
    void showReservation() {
        ObservableList<Reservation> list = (ObservableList<Reservation>) res.afficher();
        idCOL.setCellValueFactory(new PropertyValueFactory<Reservation, Integer>("idRes"));
        dateCOL.setCellValueFactory(new PropertyValueFactory<Reservation, Date>("dateRes"));
        heureCOL.setCellValueFactory(new PropertyValueFactory<Reservation, Time>("heureRes"));
        disponibleCOL.setCellValueFactory(new PropertyValueFactory<Reservation, String>("dispoRes"));
        prixCOL.setCellValueFactory(new PropertyValueFactory<Reservation, Double>("prixRes"));
      ...
      FilteredList<Reservation> filteredData = new FilteredList<>(dataList, b -> true);
        ...
        searchTF.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredData.setPredicate(reservation -> {
                ...
                } else if (String.valueOf(reservation.getDateRes()).toLowerCase().indexOf(lowerCaseFilter) != -1) {
    ...
    boolean ControlNotEmpty() {
        if (dateTF.getText().isEmpty() || heureTF.getText().isEmpty() || disponibleCB.getValue().isEmpty() || prixTF.getText().isEmpty()) {
