...
public class ReservationsCliController implements Initializable {
  ...
  @FXML private CustomTextField dateResTF;

  @FXML private CustomTextField heureResTF;

  @FXML private CustomTextField prixResTF;
  ...
  @FXML private JFXButton rateBTN;

  @FXML private ListView<Reservation> reservationCliLV;

  @FXML private JFXListView<Reservation> reservationdispoLV;
  ...
  void showMyReservation() {
    ObservableList<Reservation> listCli =
        (FXCollections.observableArrayList(res.getListResCli(r)));
  ...
  void showAvailReservation() {
    ObservableList<Reservation> list =
        (FXCollections.observableArrayList(getListResDispo()));
    reservationCliLV.getItems().clear();
  ...
  void addReservation() {
    if (!reservationdispoLV.getSelectionModel().isEmpty()) {
      Reservation reservation =
          reservationdispoLV.getSelectionModel().getSelectedItem();
  ...
  void deleteReservation() {
    if (!reservationCliLV.getSelectionModel().isEmpty()) {
      Reservation reservation =
          reservationCliLV.getSelectionModel().getSelectedItem();
  ...
  public void updateReservation(Reservation re) {
    ...
    try {
      pre = cnx.prepareStatement(
          "UPDATE reservation SET dateRes= ?,heureRes= ?,dispoRes= ?,prixRes= ?,idCli= ? where idRes = ?;");
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ObservableList<Reservation> listDispo =
        (FXCollections.observableArrayList(getListResDispo()));
