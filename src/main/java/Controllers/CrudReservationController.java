...
public class CrudReservationController implements Initializable {
  ...
  Utilisateur cli = new Utilisateur(5);
  Utilisateur coa = new Utilisateur(6);
  ...
  @FXML private ListView<Rating> ratingLV;

  @FXML private ListView<Reservation> reservationLV;
  ...
  @FXML private ComboBox<String> dispoCB;
  ...
  @FXML private JFXButton resetReservationsBTN;

  @FXML private JFXButton resetTfBTN;

  @FXML private JFXButton addBTN;

  @FXML private JFXButton ratingsBTN;
  ...
  public boolean controlNotEmpty() {
    if (dateDP.getValue() == null || heureTF.getText().isEmpty() ||
        dispoCB.getValue() == null || prixTF.getText().isEmpty()) {
      Alert alert = new Alert(Alert.AlertType.WARNING);
      alert.setTitle("Warning Alert Dialog");
      alert.setContentText("Please Verify Your Input Fields");
      alert.showAndWait();
      return false;
    } else if (!Pattern.matches("[0.0-50.0]*", prixTF.getText()) ||
               Double.valueOf(prixTF.getText()) > 50.0) {
      prixTF.requestFocus();
      prixTF.selectEnd();
      Alert alert = new Alert(Alert.AlertType.WARNING);
      alert.setTitle("Warning Alert Dialog");
      ...
      alert.showAndWait();
      return false;
    } else if ((!Pattern.matches("[00:00-23:59]*", heureTF.getText())) ||
               (!Pattern.matches("[00:00:00-23:59:00]*", heureTF.getText()))) {
      prixTF.requestFocus();
      prixTF.selectEnd();
      Alert alert = new Alert(Alert.AlertType.WARNING);
      alert.setTitle("Warning Alert Dialog");
  ...
  protected void insertReservation() {
    if (controlNotEmpty()) {
      Reservation r = new Reservation(
          Date.valueOf(dateDP.getValue()),
          Time.valueOf(LocalTime.parse(heureTF.getText())), dispoCB.getValue(),
          Double.parseDouble(prixTF.getText()), cli, coa);
      res.ajouter(r);
    ...
    if (controlNotEmpty()) {
      ...
      if (!reservationLV.getSelectionModel().isEmpty()) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
          ...
          Reservation r =
              new Reservation(Date.valueOf(dateDP.getValue()),
                              Time.valueOf(LocalTime.parse(heureTF.getText())),
                              dispoCB.getValue(),
                              Double.parseDouble(prixTF.getText()), cli, coa);
  ...
  void search() {
      ...
      filteredData.setPredicate(reservation -> {
        ...
        if (String.valueOf(reservation.getIdRes())
          ...
          return true;
                       ...
                       .indexOf(lowerCaseFilter) != -1)
          return true;
        else if (String.valueOf(reservation.getDispoRes())
                     .indexOf(lowerCaseFilter) != -1)
          return true;
        else if (String.valueOf(reservation.getPrixRes())
                     .indexOf(lowerCaseFilter) != -1)
          return true;
        else
          return false;
    ...
    sortedData.comparatorProperty().bind(
        reservationLV.getItems().sorted().comparatorProperty());
  ...
  void resetTextsFileds() {
    ...
    idTF.clear();
    dateDP.setValue(LocalDate.parse(""));
    heureTF.clear();
    dispoCB.setValue("");
    prixTF.clear();
  }
  ...
  void showRating() {
    ObservableList<Rating> list =
        FXCollections.observableArrayList(ras.getRatingCoa(r));
  ...
  public void switchScene(ActionEvent event) throws IOException {
    Parent root =
        FXMLLoader.load(getClass().getResource("reservationCrud-view.fxml"));
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ObservableList<Reservation> list =
        (FXCollections.observableArrayList(res.afficher()));
    ObservableList<Rating> list1 =
        (FXCollections.observableArrayList(ras.getRatingCoa(r)));
    reservationLV.getItems().addAll(list);
